...
export function PrayerTimes() {
  ...
  const { prayerTimes: originalPrayerTimes, error: prayerError } =
          ...
          <div className="flex flex-col items-end">
            <div className="flex space-x-2">
              ...
              >
                <Settings
                  className={`w-5 h-5 ${
                    showAdjustments ? "text-emerald-600" : "text-gray-600"
                  }`}
                />
              </button>
              <button
              ...
              >
                <Bell
                  className={`w-5 h-5 ${
                    notificationsEnabled ? "fill-emerald-600" : ""
                  }`}
                />
              </button>
            </div>
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            {Object.entries(timeAdjustments).map(([prayer, adjustment]) => (
              <TimeAdjuster
                ...
                onChange={(value) =>
                  handleAdjustment(prayer as keyof TimeAdjustments, value)
                }
              />
            ))}
          </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-gray-600">{t("common.gregorian")}</span>
              <span className="font-medium">
                {format(currentTime, "MMMM d, yyyy")}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">{t("common.islamic")}</span>
